...
- **주유소 관련 크로스체크**: pubgmaps.gg 카테고리 목록을 직접 확인한 결과 에란겔/미라마/태이고
  전부 "Fuel/Gas Station" 카테고리 자체가 없음 — BGMS(론도만 17, 나머지 0)와 정확히 일치. 초기에
  다른 웹서치 요약이 "다른 맵에도 주유소가 있다"고 시사했었는데, 실제 인터랙티브 지도 데이터
  2곳이 모두 부정하므로 그쪽이 오답이었던 것으로 판단 — **주유소는 론도 전용 유지**.
...
| ------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 에란겔 | 차고지17·고정차량41·고정보트23·비밀의방15·글라이더40·주유소0               | 전부 확보, 갭 없음                                                                                      |
| 미라마 | 고정보트10·비밀의방13·글라이더40·황금미라도2·e스포츠미라도20·e스포츠픽업20 | 글라이더 확보. 고정차량/차고지는 BGMS도 일반전 카테고리가 없어(e스포츠 룰셋만 있음) 텍스트 출처 유지 중 |
| 론도   | 차고지6·고정차량11·비밀의방15·글라이더10·주유소17                          | 전부 확보, 갭 없음                                                                                      |
| 태이고 | 차고지24·포터24(=차고지와 동일세트로 추정)·비밀의방15, 나머지 전부 0       | 전부 확보, 갭 없음                                                                                      |

**`GLIDER`/`GAS_STATION` 레이어 확보 완료(2026-08-17)**: 주유소는 조사 결과 론도의 신규 메커닉이
아니라 **모터글라이더 급유용 고정 인프라**로 확인됨(공식 패치노트 42.1/42.2엔 명시 없음 — 유튜브
숏폼·커뮤니티 위키에서 "Rondo guarantees 10 motor glider spawns at 100% rate" + "fuel stations
refuel vehicles and motor gliders" 교차 확인, 론도 글라이더 10개·주유소 17개라는 짝이 맞물림).
좌표는 기존과 동일한 방식(BGMS Leaflet 마커 생성자 몽키패치로 정확한 `{lat,lng}` 추출 → 게임
미터 좌표계 변환)으로 에란겔40·미라마40·론도10(글라이더)·론도17(주유소) 전부 BGMS 실측값과
정확히 일치하는 개수로 확보.
...
- [x] ~~`GAS_STATION`(주유소) 레이어 검토~~ — 2026-08-17 완료. 모터글라이더 급유 인프라로 확인,
      론도 17개(글라이더 10개와 짝을 이루는 메커닉), 아이콘(Fuel)+색상(`--color-marker-gas-station`)
      추가
